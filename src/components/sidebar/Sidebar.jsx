    ...
    <div className="fixed">
      <IconButton
        className={`w-12 h-12 cursor-pointer absolute rounded-[1rem] ${
          openSidebar ? "bg-[#1f2a46] left-[9rem] top-2" : ""
        } `}
      ...
      {openSidebar && (
        <div className="bg-[#1f2a46] w-[10rem] h-[87.5vh] flex flex-col">
          {sidebarData.map((data) => (
            <p className="text-gray-400 hover:text-[#FFFF] cursor-pointer hover:shadow-md pt-8 pb-0 pl-4 hover:text-xl">
              {data.text}
            </p>
          ))}
        </div>
